...
function NetlistViewerInner({
  ...
  // Layout effect: netlist structure + filters only (no activeSignals)
  useEffect(() => {
  ...
  }, [netlistJson, catFilter, textFilter]);

  // Active-state overlay: apply signal highlighting without recomputing positions
  useEffect(() => {
    if (!baseNodes.length) return;
    const hasSignals = Object.keys(activeSignals).length > 0;

    if (!hasSignals && !showOnlyActive) {
      ...
      setEdges(baseEdges.map((e) => ({
        ...
        markerEnd: { type: "arrowclosed" as any, width: 8, height: 8, color: "#cbd5e1" },
      })));
      return;
    }

    const activeKeys = Object.keys(activeSignals).map((k) => k.toLowerCase());
    const isPortActive = (name: string) =>
      activeKeys.some(
        (k) => k.endsWith(`.${name.toLowerCase()}`) || k === name.toLowerCase(),
      );

    const modName = Object.keys(netlistJson.modules ?? {})[0];
    const mod = modName ? netlistJson.modules[modName] : null;
    const activePorts = new Set<string>();
    const bitToPort: Record<string, string> = {};

    if (mod?.ports) {
      Object.entries(mod.ports).forEach(([pName, pData]: [string, any]) => {
        (pData.bits ?? []).forEach((b: any) => { bitToPort[String(b)] = pName; });
        if (isPortActive(pName)) activePorts.add(pName);
      });
    }
    ...
    const isCellActive = (cd: any) => {
      for (const bits of Object.values(cd.connections) as any[]) {
        if (!Array.isArray(bits)) continue;
        for (const b of bits) {
          if (bitToPort[String(b)] && activePorts.has(bitToPort[String(b)])) return true;
        }
      }
      return false;
    };
    ...
    if (mod?.cells) {
      Object.entries(mod.cells).forEach(([cName, cData]: [string, any]) => {
        if (isCellActive(cData)) activeCellIds.add(cName);
      });
    }

    const hasAnyActive = activePorts.size > 0 || activeCellIds.size > 0;

    const updatedNodes = baseNodes
      .filter((n) => {
        if (!showOnlyActive) return true;
        const nid = n.id;
        return nid.startsWith("port-")
          ? activePorts.has(nid.replace("port-", ""))
          : activeCellIds.has(nid);
      })
      .map((n) => {
        const nid = n.id;
        const isActive = nid.startsWith("port-")
          ? activePorts.has(nid.replace("port-", ""))
          : activeCellIds.has(nid);
        return {
          ...n,
          data: {
            ...n.data,
            isActive,
            isDimmed: hasAnyActive && !isActive,
          },
        };
      });
    ...
    const updatedEdges = baseEdges
      .filter((e) => !showOnlyActive || (activeNodeIds.has(e.source) && activeNodeIds.has(e.target)))
      .map((e) => {
        const isActive =
          activeNodeIds.has(e.source) && activeNodeIds.has(e.target) &&
          (e.data as NLEdgeData)?.busType !== undefined;
        const busType = (e.data as NLEdgeData)?.busType ?? "wire";
        return {
          ...e,
          data: { ...e.data, isActive },
          markerEnd: {
            type: "arrowclosed" as any,
            width: 8,
            height: 8,
            color: isActive ? (BUS_COLORS[busType] ?? "#cbd5e1") : "#cbd5e1",
          },
        };
      });
  ...
  }, [baseNodes, baseEdges, activeSignals, showOnlyActive, netlistJson, setNodes, setEdges]);
